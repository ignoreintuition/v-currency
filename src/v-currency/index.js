...
function convert(o, val) {
  if (o.type === undefined) return `${val.toString()}`;
  if (RULES[o.type] === undefined)
    return Number(val).toLocaleString(navigator.language || "en-US", {
      style: "currency",
      currency: o.type,
    });
  val = o.thousandSeparator
    ? insertThousandSeparator(val, RULES[o.type].thousandSeperator)
    : val;
...
function insertCurrencySymbol(val, format) {
  ...
  if (format.indexOf("_") > -1) {
    ...
    val =
      format.indexOf("_") === 0
        ? `${pSign + val.toString() + " " + format.substr(1, format.length)}`
        : `${pSign + format.substr(0, format.length - 1) + " " + val.toString()}`;
  } else val = `${format + " " + val.toString()}`;
  return val;
}
